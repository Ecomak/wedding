    ...
    <div className="min-h-screen flex flex-col items-center">
      ...
      <main className="w-full max-w-5xl px-4 -mt-12 mb-20 relative z-10">
        <div className="flex flex-col gap-20">
          ...
          {/* --- ONLY THIS SECTION IS HIDDEN --- */}
          <section id="schedule" className="hidden">
          ...
          <section id="location" className="text-center scroll-mt-24">
            ...
            <div className="p-8 bg-blue-50/30 rounded-3xl border border-blue-100 text-left">
              ...
              <div className="mt-6 flex flex-wrap gap-4">
                 <a href="https://www.sbb.ch/" target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-white border border-[#A1B5C1] text-[#A1B5C1] rounded-full text-xs font-bold hover:bg-[#A1B5C1] hover:text-white transition-all shadow-sm">SBB.ch</a>
              </div>
            </div>
          </section>
          ...
          <section id="attractions" className="text-center scroll-mt-24">
                ...
                <ul className="space-y-3">
                  {t.zurichAttractions.map((item, idx) => (
                    <li key={idx} className="text-gray-600 text-sm flex items-start gap-2">
                      <span className="text-[#BFAE90]">•</span> {item}
                    </li>
                  ))}
                </ul>
              </div>
            ...
            <div className="mt-8 p-6 bg-gray-50 rounded-2xl border border-gray-100 text-left">
               <h3 className="text-lg font-bold text-[#BFAE90] mb-3 flex items-center gap-2">
                 <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5"><path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5c-3.162 0-6.133-.815-8.716-2.247m0 0A9.015 9.015 0 013 12c0-.856.12-1.685.344-2.47" /></svg>
                 {t.eatTitle}
               </h3>
               <div className="text-gray-600 text-sm space-y-4">
                 <p>{lang === 'TR' ? 'İsviçre denince akla gelen klasik lezzetleri mutlaka deneyin: ' : 'Don\'t miss out on these essential Swiss classics: '}
                   <span className="font-bold text-gray-800">Fondue</span>, <span className="font-bold text-gray-800"> Raclette</span>, <span className="font-bold text-gray-800"> Rösti</span>
                   {lang === 'TR' ? ' ve meşhur İsviçre peynirleri.' : ' and of course, famous Swiss cheeses.'}
                 </p>
                 <p>{lang === 'TR' ? 'Çikolata severler için ' : 'For chocolate lovers, a visit to '}
                   <span className="font-bold text-mountain">Läderach</span> {lang === 'TR' ? ' veya ' : ' or '} <span className="font-bold text-mountain">Sprüngli</span> 
                   {lang === 'TR' ? ' (meşhur Luxemburgerli makaronları ile) butikleri tatlı bir mola için idealdir.' : ' (try their famous Luxemburgerli macarons) boutiques is a must for a sweet break.'}
                 </p>
               </div>
            </div>
          </section>
